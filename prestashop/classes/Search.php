...
class Search
{
	...
	/*-------------------------------advsearch---------------------------*/
	public function find_adv($id_lang, $expr, $pageNumber = 1, $pageSize = 10, $orderBy = false, $orderWay = false)
	{
		...
		$displaysupp=Search::getSuppliers(Tools::getValue('supply'),'id_supplier');
		$displaymanuf=Search::getSuppliers(Tools::getValue('manufacturer'),'id_manufacturer');
		...
		$pos = strpos($attrib, ',');
				...
				$attributes=Search::attributesQuery($id_lang,$att,true);
				$attributes_val=Search::attributesQuery($id_lang,$att,false);
			
		}else{	
			if($attribu[0]>0){
				$attributes=Search::attributesQuery($id_lang,$attribu,true);
				$attributes_val=Search::attributesQuery($id_lang,$attribu,false);
			}else{
				$attributes='';
				$attributes_val='';
			}
		...
		$displaycategories=Search::queryCategories(Tools::getValue('categories'));
...
END  ";
		...
		$sql='SELECT DISTINCT p.id_product,'.(((Tools::getValue('price_from')>0)||(Tools::getValue('price_to')>0))? $price_brutto.' as price_brutto,':'').' p.*, pl.`description_short`, pl.`available_now`, pl.`available_later`, pl.`link_rewrite`, pl.`name`, t.`rate`, i.`id_image`, il.`legend` 
		FROM `'._DB_PREFIX_.'product` p
	...
	/*-------------------------------advsearch---------------------------*/
	public function categoriesLower($parent){
	...
	if($ile>0){
		while($row = mysql_fetch_array($result)){
			...
			$display.=Search::categoriesLower($row['id_category']);
		}
	}	
	
	return $display; 
	}
	public function queryCategories($cats){
		$categs='';
		if(!is_array($cats)){
			$cats=urldecode($cats);
			$pos = strpos($cats, ',');
		...
		if($cats[0]>0){
			foreach($cats as $key=>$val){
				$categs.=$val.',';
				$categs.= Search::categoriesLower($val);
	...
	public function attributesQuery($lang,$id,$join=true){
		if($join==true){
	...
	public function getSuppliers($suppliers,$table_val){
	$displaysupp='';
