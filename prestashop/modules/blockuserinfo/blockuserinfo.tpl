	...
	<ul id="header_nav">
		<li id="shopping_cart">
			<a href="{$base_dir_ssl}order.php" title="{l s='Your Shopping Cart' mod='blockuserinfo'}">{l s='Cart:' mod='blockuserinfo'}</a>
				...
				<span class="ajax_cart_total{if $cart_qties == 0} hidden{/if}">{if $cart_qties > 0}{if $priceDisplay == 1}{convertPrice price=$cart->getOrderTotalLC(false, 4)}{else}{convertPrice price=$cart->getOrderTotalLC(true, 4)}{/if}{/if}</span>
