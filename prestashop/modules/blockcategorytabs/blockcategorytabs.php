...
class BlockCategoryTabs extends Module
{
	function __construct()
	{
		$this->name = 'blockcategorytabs';
		$this->tab = 'Blocks';
		$this->version = 1.0;
		...
		$this->description = $this->l('Adds a tab-bar of the top-level product categories and a block featuring sub categories');
	}
	...
	public function getContent()
	{
		...
		if (Tools::isSubmit('submitBlockCategoryTabs'))
		{
	...
	function findCurrentCategoryPath($blockCategTree, $currentCategoryId)
	{
		if ($blockCategTree['id'] == $currentCategoryId)
		        return array($currentCategoryId);
		foreach ($blockCategTree['children'] as $child) {
			$path = $this->findCurrentCategoryPath($child, $currentCategoryId);
			if ($path != null) {
			   	  array_push($path, $blockCategTree['id']);
				  return $path;
			}
		}
		return array();
	...
	function hookTop($params) {
		...
		return $this->display(__FILE__, 'toptabs.tpl');
	}
	...
	function hookLeftColumn($params)
	{
		...
		return $this->display(__FILE__, 'sideblock.tpl');
	}
