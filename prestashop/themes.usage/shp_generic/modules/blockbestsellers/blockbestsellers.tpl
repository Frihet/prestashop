	...
	{if $best_sellers|@count > 0}
		...
		{foreach from=$best_sellers item='product' name=product}
			...
			<tr class="ajax_block_product">
				...
				<td rowspan="2"><a class="button_mini ajax_add_to_cart_button" href="{$base_dir}cart.php?qty=1&amp;id_product={$product.id_product|intval}&amp;token={$static_token}&amp;add" rel="ajax_id_product_{$product.id_product|intval}" title="{l s='Add to cart' mod='blockbestsellers'}">{l s='Buy' mod='blockbestsellers'}</a></td>
			</tr>
			<tr>
				<td>{$product.price}</td>
			</tr>
			...
			<tr class="ajax_block_product">
				...
				<td>{$product.price}</td>
				<td><a class="button_mini ajax_add_to_cart_button" href="{$base_dir}cart.php?qty=1&amp;id_product={$product.id_product|intval}&amp;token={$static_token}&amp;add" rel="ajax_id_product_{$product.id_product|intval}" title="{l s='Add to cart' mod='blockbestsellers'}">{l s='Buy' mod='blockbestsellers'}</a></td>
			</tr>
			-->
		{/foreach}
		</table>
