...
<div id="cart_block" class="universal block">
	...
	<div id="cart_block_summary" class="{if $colapseExpandStatus eq 'expanded' || !$ajax_allowed}collapsed{else}expanded{/if}">
		...
		<span class="ajax_cart_total">{if $cart_qties > 0}{if $priceDisplay == 1}{convertPrice price=$cart->getOrderTotalLC(false)}{else}{convertPrice price=$cart->getOrderTotalLC(true)}{/if}{/if}</span>
	...
	<div class="block_buttons">
		...
		-->
		<a href="{$base_dir_ssl}order.php" id="button_order_cart" class="exclusive_right" title="{l s='Check out' mod='blockcart'}">{l s='Check out' mod='blockcart'}</a>
	</div>
</div>
<!-- /MODULE Block cart -->
