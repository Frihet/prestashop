...
class NetaxeptDKK extends PaymentModule
{
	private	$_html = '';
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('Netaxept_dkk');
	...
	{
		return Configuration::get('NETAXEPT_DKK_TERMINAL_URL');
	}
	...
	{
		return Configuration::get('NETAXEPT_DKK_WSDL_URL');
	}
	...
	{
		return Configuration::get('NETAXEPT_DKK_TQ_WSDL_URL');
	}
	...
	{
		if (!parent::install()
			OR !Configuration::updateValue('NETAXEPT_DKK_MID_TOKEN_CURR', '10000327;5e_R-Zp9;NOK')
			OR !Configuration::updateValue('NETAXEPT_DKK_TERMINAL_URL', 'https://epayment-test.bbs.no/terminal/default.aspx')
			OR !Configuration::updateValue('NETAXEPT_DKK_WSDL_URL', 'https://epayment-test.bbs.no/service.svc?wsdl')
			OR !Configuration::updateValue('NETAXEPT_DKK_TQ_WSDL_URL', 'https://epayment-test.bbs.no/TokenQuery.svc?wsdl')
			OR !$this->registerHook('paymentDKK')
			OR !$this->registerHook('paymentDKKReturn'))
			return false;
	...
	{
		if (!Configuration::deleteByName('NETAXEPT_DKK_MID_TOKEN_CURR')
			OR !Configuration::deleteByName('NETAXEPT_DKK_TERMINAL_URL')
			OR !Configuration::deleteByName('NETAXEPT_DKK_WSDL_URL')
			OR !Configuration::deleteByName('NETAXEPT_DKK_TQ_WSDL_URL')
			OR !parent::uninstall())
			return false;
	...
	public function getContent()
	{
		...
		{
				Configuration::updateValue('NETAXEPT_DKK_MID_TOKEN_CURR', strval($_POST['mid_token_curr']));
				Configuration::updateValue('NETAXEPT_DKK_TERMINAL_URL', strval($_POST['terminal_url']));
				Configuration::updateValue('NETAXEPT_DKK_WSDL_URL', strval($_POST['wsdl_url']));
				Configuration::updateValue('NETAXEPT_DKK_TQ_WSDL_URL', strval($_POST['tq_wsdl_url']));
				$this->displayConf();
		} else {
			$this->displayErrors();
		}
	...
	{
		$conf = Configuration::getMultiple(array('NETAXEPT_DKK_MID_TOKEN_CURR', 'NETAXEPT_DKK_TERMINAL_URL', 'NETAXEPT_DKK_WSDL_URL', 'NETAXEPT_DKK_TQ_WSDL_URL'));
		$mid_token_curr = array_key_exists('uid', $_POST) ? $_POST['mid_token_curr'] : (array_key_exists('NETAXEPT_DKK_MID_TOKEN_CURR', $conf) ? $conf['NETAXEPT_DKK_MID_TOKEN_CURR'] : '');
		$terminal_url = array_key_exists('terminal_url', $_POST) ? $_POST['terminal_url'] : (array_key_exists('NETAXEPT_DKK_TERMINAL_URL', $conf) ? $conf['NETAXEPT_DKK_TERMINAL_URL'] : '');
		$wsdl_url = array_key_exists('wsdl_url', $_POST) ? $_POST['wsdl_url'] : (array_key_exists('NETAXEPT_DKK_WSDL_URL', $conf) ? $conf['NETAXEPT_DKK_WSDL_URL'] : '');
		$tq_wsdl_url = array_key_exists('tq_wsdl_url', $_POST) ? $_POST['tq_wsdl_url'] : (array_key_exists('NETAXEPT_DKK_TQ_WSDL_URL', $conf) ? $conf['NETAXEPT_DKK_TQ_WSDL_URL'] : '');
	...
	{
		$mid_token_curr = Configuration::get('NETAXEPT_DKK_MID_TOKEN_CURR');
		$arr = explode("\n", $mid_token_curr);
	...
	public function hookPayment($params)
	{
		...
		$NETAXEPT_DKK_url 			= Configuration::get('NETAXEPT_DKK_TERMINAL_URL');
		// FIXME: https
		$redirect_url 			= 'http://'.htmlspecialchars($_SERVER['HTTP_HOST'], ENT_COMPAT, 'UTF-8').__PS_BASE_URI__.'modules/netaxept_dkk/validation.php';
...
die(1);
		...
		$smarty->assign(array(
							  ...
							  'NETAXEPT_DKK_url' => $NETAXEPT_DKK_url));

		return $this->display(__FILE__, 'netaxept_dkk.tpl');
	}
