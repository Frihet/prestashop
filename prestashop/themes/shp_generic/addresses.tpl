...
<div class="addresses">
	...
	<table class="std">
		{foreach from=$addresses item=address name=myLoop}
		        <thead><tr><th>{$address.alias}</th></tr></thead>
		        <tbody><tr><td>
				...
				<a class="button" href="{$base_dir_ssl}address.php?id_address={$address.id_address|intval}" title="{l s='Update'}">{l s='Update'}</a>
				<a class="button" href="{$base_dir_ssl}address.php?id_address={$address.id_address|intval}&amp;delete" onclick="return confirm('{l s='Are you sure?'}');" title="{l s='Delete'}">{l s='Delete'}</a>
			</td></tr></tbody>
		{/foreach}
	</table>
</div>
